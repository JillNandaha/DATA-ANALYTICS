...
TRUNCATE TABLE assignment.inventory;
...
select count(product_id) from assignment.Sales;
...
select c.customer_id, concat(c.first_name, ' ', c.last_name ) as full_name, SUM(s.total_amount) as total_spent
from assignment.customers c 
...
SELECT 
    ...
    c.first_name AS customer1_first,
    c.last_name AS customer1_last,
...
select count(distinct s.customer_id) as customer
from assignment.sales s 
join assignment.products p on s.product_id = p.product_id
where p.price > 500;
...
select count(s.sale_id) as sale_count
from assignment.sales s 
...
SELECT p.product_id,p.product_name, AVG(quantity_sold) AS avg_quantity_sold
FROM assignment.sales s
join assignment.products p on s.product_id = p.product_id 
GROUP BY p.product_id, p.product_name;
...
where i.stock_quantity < 5
  and p.product_id in (select distinct product_id from assignment.sales s );
...
SELECT 'Highest Stock' AS stock_level, product_name, stock_quantity
FROM assignment.products
WHERE stock_quantity = (SELECT MAX(stock_quantity) FROM assignment.products)
UNION ALL
SELECT 'Lowest Stock', product_name, stock_quantity
FROM assignment.products
...
select CONCAT(c.first_name, ' ', c.last_name) AS full_name,
       p.product_name, SUM(s.total_amount) AS total_sales_amount
from assignment.customers c
...
select p.product_id, p.product_name,p.price, sum(s.quantity_sold) as total_sold
from assignment.products p 
join assignment.sales s on p.product_id = s.product_id
where p.price >200
...
having SUM(s.total_amount) > (select AVG(customer_total)
	from (select SUM(s.total_amount ) as customer_total
	from assignment.sales s
...
where c.customer_id not in (select distinct c.customer_id from assignment.sales s );
...
select * from assignment.products p 
where p.product_id not in (select distinct p.product_id from assignment.sales s );



-- 55. Which customer made the single most expensive purchase?
select c.customer_id, concat(c.first_name, ' ', c.last_name) as full_name, s.total_amount
from assignment.customers c 
join assignment.sales s on c.customer_id = s.customer_id 
where s.total_amount = (select MAX(s.total_amount ) from assignment.sales s);
...
from 
	(select SUM(s.total_amount ) as product_total
	from assignment.sales s
	group by product_id 
	)t
);
...
(select to_timestamp(AVG(extract (epoch from c2.registration_date )))
	from assignment.customers c2 
);
...
order by highest_revenue.revenue desc
limit 1;
...
select c.customer_id, CONCAT(c.first_name, ' ', c.last_name) AS full_name, pc.purchase_count
...
with total_spent as (
select c.customer_id, sum(s.total_amount) as total_spe
from assignment.customers c 
join assignment.sales s on c.customer_id = s.customer_id
group by c.customer_id 
),
avg_spent as (
select AVG(total_spe) as avg_spe
from total_spent 
)
select c.customer_id, concat(c.first_name, ' ', c.last_name) as full_name, ts.total_spe
from assignment.customers c 
join total_spent ts on ts.customer_id = c.customer_id 
join avg_spent on ts.total_spe > avg_spent.avg_spe ;
...
select p.product_id, p.product_name, SUM(s.total_amount) as total_sold
from assignment.products p 
join assignment.sales s on p.product_id = s.product_id
group by p.product_id , p.product_name 
)
select * from total_revenue 
order by total_revenue.total_sold desc;
...
select product_id, count(distinct s.customer_id ) as unique_customers
from assignment.sales s 
group by product_id 
)
select p.product_name, ps.unique_customers
from product_sale ps
join assignment.products p on ps.product_id = p.product_id
where ps.unique_customers > 3;
...
select p.product_id, p.product_name, tq.total_sold
from assignment.products p 
join total_quantity tq on p.product_id = tq.product_id 
join avg_quantity aq on tq.total_sold < aq.avg_sold ;
...
SELECT c.customer_id, CONCAT(c.first_name, ' ', c.last_name) AS full_name,
       p.product_name, COUNT(*) AS times_purchased
FROM assignment.customers c
JOIN assignment.sales s ON c.customer_id = s.customer_id
JOIN assignment.products p ON s.product_id = p.product_id
...
WITH customer_qty AS (
    ...
    FROM assignment.sales GROUP BY customer_id
)
SELECT c.customer_id, CONCAT(c.first_name, ' ', c.last_name) AS full_name,
       cq.total_qty
FROM customer_qty cq
JOIN assignment.customers c ON cq.customer_id = c.customer_id
...
SELECT * FROM spending WHERE percentile_group = 1;
...
FROM ranked
WHERE running_revenue <= grand_total * 0.5;
...
SELECT DISTINCT c.customer_id, CONCAT(c.first_name, ' ', c.last_name) AS full_name
FROM with_next wn
JOIN assignment.customers c ON wn.customer_id = c.customer_id
...
WITH tier_avg AS (
    SELECT c.membership_status, AVG(s.total_amount) AS avg_tier_spending
    FROM assignment.customers c
    JOIN assignment.sales s ON c.customer_id = s.customer_id
...
WITH category_avg AS (
    SELECT p.category, AVG(s.total_amount) AS avg_category_sales
    FROM assignment.products p
    JOIN assignment.sales s ON p.product_id = s.product_id
    GROUP BY p.category
),
...
SELECT * FROM ratios
ORDER BY purchase_ratio DESC LIMIT 1;
...
WITH category_sales AS (
    SELECT p.category, p.product_name, SUM(s.quantity_sold) AS total_qty,
           DENSE_RANK() OVER (PARTITION BY p.category
                              ORDER BY SUM(s.quantity_sold) DESC) AS rnk
    FROM assignment.products p
    JOIN assignment.sales s ON p.product_id = s.product_id
    GROUP BY p.category, p.product_name
)
SELECT * FROM category_sales WHERE rnk <= 3;
...
WITH years AS (
    SELECT DISTINCT EXTRACT(YEAR FROM sale_date) AS sale_year FROM assignment.sales
),
product_years AS (
    SELECT product_id, EXTRACT(YEAR FROM sale_date) AS sale_year
    FROM assignment.sales GROUP BY product_id, sale_year
)
SELECT p.product_name
FROM assignment.products p
WHERE (
    SELECT COUNT(DISTINCT py.sale_year) FROM product_years py
    WHERE py.product_id = p.product_id
